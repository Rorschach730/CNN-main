...
class JiT(nn.Module):
    def __init__(self, input_size=256, patch_size=16, in_channels=4, out_channels=1, hidden_size=768, depth=12,
        ...
        # [架构升级]: 剂量比例的独立物理嵌入器 (Dose Embedder)
        self.dose_embedder = nn.Sequential(
            nn.Linear(1, 256),
            nn.SiLU(),
    ...
    def initialize_weights(self):
        ...
        # 初始化新增的剂量嵌入器参数
        nn.init.normal_(self.dose_embedder[0].weight, std=0.02)
        nn.init.normal_(self.dose_embedder[2].weight, std=0.02)
    ...
    # [架构升级]: 新增 dose_ratio 参数接收
    def forward(self, x, t, dose_ratio):
        t_emb = self.t_embedder(t)

        # 提取剂量嵌入（取对数域对网络更友好，防止极小值梯度消失）
        # 加上 1e-5 防止 log(0)
        dose_log = torch.log(dose_ratio + 1e-5)
        if dose_log.dim() == 1:
            dose_log = dose_log.unsqueeze(1)

        dose_emb = self.dose_embedder(dose_log)

        # [核心融合]：时间上下文与剂量上下文物理叠加
        c = t_emb + dose_emb

...
class VisionRotaryEmbeddingFast(nn.Module):
    def __init__(
        ...
        if num_cls_token > 0:
            ...
            cos_pad = torch.ones(num_cls_token, D, dtype=cos_img.dtype) # 移除 device
            sin_pad = torch.zeros(num_cls_token, D, dtype=sin_img.dtype) # 移除 device

            # 使用 register_buffer 替代直接 .cuda()

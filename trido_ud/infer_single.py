...
def get_args():
    ...
    parser.add_argument('--ckpt_path', type=str, default='./trido_output/checkpoint-50.pth')
...
def main():
    ...
    if 'model_ema' in ckpt:
        try:
            model.net.load_state_dict(ckpt['model_ema'])
    ...
    # 1. Image Domain
    axes[0, 0].imshow(img_cond, cmap='gray', vmin=0, vmax=vmax); axes[0, 0].set_title("Input (Low Dose Image)")
    axes[0, 1].imshow(img_pred, cmap='gray', vmin=0, vmax=vmax); axes[0, 1].set_title("Denoised (TriDo-JiT)", color='green')
    axes[0, 2].imshow(img_targ, cmap='gray', vmin=0, vmax=vmax); axes[0, 2].set_title("Target (Full Dose Image)")
    ...
    plt.suptitle(f"TriDo-JiT (Fast Single View) | ID: {idx} | Anatomy: {bp_name}", fontsize=16, fontweight='bold', y=0.95)

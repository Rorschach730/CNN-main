...
def main():
    ...
    with torch.no_grad():
        output_tensor = model.generate(condition_t.unsqueeze(0).to(device), dose_t.unsqueeze(0).to(device),
    ...
    if InferenceConfig.use_roi:
        eval_gt_raw, eval_in_raw, eval_out_raw = gt_raw[rmin:rmax, cmin:cmax], in_raw[rmin:rmax, cmin:cmax], out_raw[
            rmin:rmax, cmin:cmax]
        title_suffix = "(ROI)"
    else:
        eval_gt_raw, eval_in_raw, eval_out_raw = gt_raw, in_raw, out_raw
        title_suffix = "(Whole Image)"

    str_in = f"Noisy Input {title_suffix}\n"
    str_out = f"JiT Denoised {title_suffix}\n"

    if InferenceConfig.calc_bias:
        bias_in = calculate_suv_bias(eval_gt_raw, eval_in_raw)
        bias_out = calculate_suv_bias(eval_gt_raw, eval_out_raw)
        str_in += f"SUVmax Bias(98%): {bias_in:.2f}%\n"
        str_out += f"SUVmax Bias(98%): {bias_out:.2f}%\n"

    if InferenceConfig.calc_cv:
        # 【核心改动】：动态计算当前切片的真实量程，避免大脑高亮区域截断失效
        ...
        psnr_out = compare_psnr(eval_gt_raw, eval_out_raw, data_range=dyn_range)
        ssim_out = compare_ssim(eval_gt_raw, eval_out_raw, data_range=dyn_range)
        snr_out = calculate_snr(eval_gt_raw, eval_out_raw)
        mse_out = np.mean((eval_gt_raw - eval_out_raw) ** 2)

        str_in += f"PSNR: {psnr_in:.2f} | SSIM: {ssim_in:.3f} | SNR: {snr_in:.2f} | MSE: {mse_in:.4f}\n"
        str_out += f"PSNR: {psnr_out:.2f} | SSIM: {ssim_out:.3f} | SNR: {snr_out:.2f} | MSE: {mse_out:.4f}\n"

    # 画图依然用 clip，因为显示器只支持 RGB 映射
    vmin, vmax = robust_windowing(gt_raw)
    fig, axes = plt.subplots(1, 3, figsize=(15, 6))
    fig.suptitle(f"[{f_name}]  |  Dose Ratio: {true_dose:.3f}", fontsize=14, fontweight='bold')

    axes[0].imshow(np.clip(in_raw, 0, vmax), cmap='gray', vmin=vmin, vmax=vmax)
    axes[0].set_title(str_in.strip(), fontsize=10);
    axes[0].axis('off')

    axes[1].imshow(np.clip(out_raw, 0, vmax), cmap='gray', vmin=vmin, vmax=vmax)
    axes[1].set_title(str_out.strip(), fontsize=10);
    axes[1].axis('off')

    axes[2].imshow(np.clip(gt_raw, 0, vmax), cmap='gray', vmin=vmin, vmax=vmax)
    ...
    np.save(os.path.join(save_dir, f"{base_name}_roi_out.npy"), eval_out_raw)
    np.save(os.path.join(save_dir, f"{base_name}_roi_in.npy"), eval_in_raw)
    print(f"[*] 原始 ROI 矩阵已落盘至: {save_dir}")

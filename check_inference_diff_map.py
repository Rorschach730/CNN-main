...
def main():
    ...
    # 1. 寻找匹配的基准图像 (Matched Image)
    matched_label = next((k for k in InferenceConfig.test_dose_labels if abs(k - true_dose) < 1e-4), None)
    if matched_label is None:
        raise ValueError(f"[!] 测试标签池不包含当前的真实剂量 {true_dose}，无法执行相减对照。")

    matched_out_raw = out_raw_dict[matched_label]

    # 2. 计算差异矩阵，并获取全局最大绝对差异以对称设置 Colorbar
    diff_raw_dict = {}
    max_abs_diff = 1e-5
    for k in InferenceConfig.test_dose_labels:
        if k == matched_label:
            diff_raw_dict[k] = matched_out_raw  # 匹配组保留原灰度图
        else:
            diff_raw = out_raw_dict[k] - matched_out_raw
            diff_raw_dict[k] = diff_raw
            max_abs_diff = max(max_abs_diff, np.max(np.abs(diff_raw)))
    ...
    for k in InferenceConfig.test_dose_labels:
        if k == matched_label:
            str_out_dict[k] = f"Matched Denoised\n(Label: {k:.3f})\n"
        else:
            str_out_dict[k] = f"Diff: Mismatch - Match\n(Label: {k:.3f} - {matched_label:.3f})\n"
    ...
    if InferenceConfig.calc_bias:
        ...
        for k, eval_out in eval_out_dict.items():
            ...
            str_out_dict[k] += f"SUVmax Bias: {bias_out:.2f}%\n"
    ...
    if InferenceConfig.calc_cv:
        ...
        str_in += f"PSNR: {psnr_in:.2f} | SSIM: {ssim_in:.3f}\nSNR: {snr_in:.2f} | MSE: {mse_in:.4f}\n"
        ...
        for k, eval_out in eval_out_dict.items():
            ...
            str_out_dict[k] += f"PSNR: {psnr_out:.2f} | SSIM: {ssim_out:.3f}\nSNR: {snr_out:.2f} | MSE: {mse_out:.4f}\n"
    ...
    for k in InferenceConfig.test_dose_labels:
        if k == matched_label:
            # 匹配组：展示常规去噪图
            im = axes[plot_idx].imshow(np.clip(diff_raw_dict[k], 0, vmax), cmap='gray', vmin=vmin, vmax=vmax)
        else:
            # 错配组：展示发散色带的相减差异图
            im = axes[plot_idx].imshow(diff_raw_dict[k], cmap='bwr', vmin=-max_abs_diff, vmax=max_abs_diff)
            # 添加刻度条以量化误差
    ...
    save_dir = "./npy_results_probe_diff"
    os.makedirs(save_dir, exist_ok=True)
    base_name = f_name.replace('.pt', '')

    # 将 ROI 相减后的差异矩阵落盘，供可能需要的数值分析使用
    for k, diff_raw in diff_raw_dict.items():
        if k != matched_label:
            eval_diff = diff_raw[rmin:rmax, cmin:cmax] if InferenceConfig.use_roi else diff_raw
            np.save(os.path.join(save_dir, f"{base_name}_diff_D{k:.3f}_minus_Match.npy"), eval_diff)

    print(f"[*] 差异图渲染完毕，相减矩阵已落盘至: {save_dir}")
